...
module.exports = class JpegXmpParser {
  ...
  static write (srcPath, dstPath, xmp) {
    if (typeof(xmp) === 'object') {
      let builder = new xml2js.Builder()
      let xml = builder.buildObject(xmp)
      xml = xml.split('\n')
      xml.shift()  // <?xml version="1.0" encoding="UTF-8" standalone="yes"?> 削除
      xml.unshift('<?xpacket begin="" id="W5M0MpCehiHzreSzNTczkc9d"?>')
      xml.push('<?xpacket end="r"?>')
      xml.push('')
      xmp = xml.join('\n')
    }
    ...
    if (position) {
      ...
      fs.closeSync(fd)
      fs.writeFileSync(dstPath, this._createBufferWithXmp(xmp, srcPath, position, position + size + 2))
    } else {
  ...
  static parseToObject(filePath) {
    let string = this.parse(filePath)
    let object = null

    if (string) {
      xml2js.parseString(string, (error, result) => {
        if (error) {
          throw error
        } else {
          object = result
